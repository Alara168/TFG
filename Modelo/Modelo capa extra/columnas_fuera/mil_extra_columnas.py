...
# ==========================================
# 2. MODELO: GATED ATTENTION MIL (MODIFICADO)
# ==========================================
class GatedAttentionMIL(nn.Module):
    def __init__(self, input_dim, num_classes):
        super(GatedAttentionMIL, self).__init__()
        self.L = 512  # Aumentamos la dimensión interna
        self.D = 128
        
        # Añadida Capa Extra: de input_dim -> 512 -> 256
        self.feature_extractor = nn.Sequential(
            nn.Linear(input_dim, self.L),
            nn.ReLU(),
            nn.Dropout(0.35),
            nn.Linear(self.L, 256), # Capa extra añadida
            nn.ReLU(),
            nn.Dropout(0.35),
            nn.Linear(256, 256),
            nn.ReLU()
        ...
        self.attention_V = nn.Sequential(nn.Linear(256, self.D), nn.Tanh())
        self.attention_U = nn.Sequential(nn.Linear(256, self.D), nn.Sigmoid())
...
def plot_architecture(input_dim):
    ...
    x = range(len(layers))
    y = [1] * len(layers)
    plt.plot(x, y, 'ko-', markersize=40, markerfacecolor='lightgreen') # Cambiado color para distinguir
    for i, txt in enumerate(layers):
        plt.annotate(txt, (x[i], y[i]), textcoords="offset points", xytext=(0,10), ha='center', fontweight='bold')
    plt.title("Arquitectura Profunda Gated Attention MIL")
    plt.axis('off')
...
def entrenar(csv_path, k_folds=5):
    ...
    with open('estadisticas_modelo.txt', 'w') as f: f.write(report)
    print(f"\n{report}")
    ...
    for i in range(min(2, len(test_idx))):
        ...
        for n, p in zip(NOMBRES_CLASES, probs_f): print(f"Fichero -> {n}: {p*100:.2f}%")
...
if __name__ == "__main__":
    ...
    if os.path.exists(csv_file): entrenar(csv_file)